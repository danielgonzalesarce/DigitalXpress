...
<body>
    ...
    <script>
        // ============================================
        // ANIMACIÓN DE BATERÍA PARA LOGIN/LOGOUT
        // ============================================
        
        // Detectar si hay mensaje de éxito de autenticación al cargar
        document.addEventListener('DOMContentLoaded', function() {
            const authSuccessMessage = document.querySelector('.alert-success[data-auth-message="true"]');
            if (authSuccessMessage) {
                showBatteryAnimation('loading', 'Cargando...');
            }
            ...
            if (logoutForm) {
                logoutForm.addEventListener('submit', function(e) {
                    ...
                    showBatteryAnimation('unloading', 'Saliendo de la página...', action);
                });
            }
        });

        function showBatteryAnimation(type, message, redirectUrl = null) {
            ...
            // Configurar mensaje
            batteryMessage.textContent = message;
            
            // Mostrar overlay
            overlay.style.display = 'flex';
            
            if (type === 'loading') {
                // Animación de carga (0% a 100% en verde)
                batteryLevel.classList.remove('discharging');
                batteryLevel.classList.add('charging');
                batteryPercentage.classList.remove('discharging');
                batteryPercentage.style.color = '#10b981';
                ...
                const interval = setInterval(() => {
                    ...
                    if (percentage >= 100) {
                        ...
                        setTimeout(() => {
                            ...
                            batteryPercentage.textContent = '0%';
                        }, 500);
                    }
                }, 30); // 30ms * 50 = 1500ms total
                
            } else if (type === 'unloading') {
                // Animación de descarga (100% a 0% de verde a rojo)
                batteryLevel.style.height = '100%';
                batteryLevel.style.background = 'linear-gradient(to top, #10b981 0%, #34d399 100%)';
                batteryPercentage.textContent = '100%';
                batteryPercentage.style.color = '#10b981';
                
                setTimeout(() => {
                    batteryLevel.classList.remove('charging');
                    batteryLevel.classList.add('discharging');
                    batteryPercentage.classList.add('discharging');
                    ...
                    const interval = setInterval(() => {
                        ...
                        if (percentage <= 50) {
                            const redIntensity = (50 - percentage) / 50;
                            const greenIntensity = percentage / 50;
                            batteryLevel.style.background = `linear-gradient(to top, 
                                rgb(${Math.round(239 * redIntensity + 16 * greenIntensity)}, 
                                    ${Math.round(68 * redIntensity + 185 * greenIntensity)}, 
                                    ${Math.round(68 * redIntensity + 129 * greenIntensity)}) 0%, 
                                rgb(${Math.round(248 * redIntensity + 52 * greenIntensity)}, 
                                    ${Math.round(113 * redIntensity + 211 * greenIntensity)}, 
                                    ${Math.round(113 * redIntensity + 153 * greenIntensity)}) 100%)`;
                            batteryPercentage.style.color = `rgb(${Math.round(239 * redIntensity + 16 * greenIntensity)}, 
                                ${Math.round(68 * redIntensity + 185 * greenIntensity)}, 
                                ${Math.round(68 * redIntensity + 129 * greenIntensity)})`;
                        }
